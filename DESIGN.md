...
The report answers all three questions.
...
with exactly one skill available (none for the baseline). The prompts are written as unambiguous
...
harness's skill support, which matters for portability. `inline` runs are judged like the others.
...
- `harness-stamp`: whether the harness supports hooks, subagents, image generation.
...
1. **Skill loaded**: transcript shows the skill being invoked or `SKILL.md` being read, or
   ...
   run if the skill was loaded, so this is reported as a precondition.
2. **Deterministic outcome**: files on disk after the run, stamp values against ground truth,
   branch file present, feature and tests executed, delegation call seen in the transcript.
3. **Adherence (LLM judge)**: Fable 5.1 grades every valid skill run: format followed, no
   unrequested actions, claims match actions. 0 to 2 each. Baseline runs are not judged, since
   there is no skill to grade against.
...
4 subjects x 3 delivery x (6 + 9 prompts) x 2 reps = 360 skill runs, plus 4 subjects x 6 baseline
prompts x 2 reps = 48 baseline runs, 408 in total. `bin/run.py` accepts filters so a subset can be
run first: `--delivery inline` for the inline control, `--prompts G1 G2 G3` for one skill,
`--prompts B1 B2 B3` or `--prompts BR1 BR2 BR3` for a baseline. Which runs were made when is
...
bin/judge.py                                                        # judge.json per run
...
- the **question** it answers: branching, portability or efficiency (see Goal);
...
`reported_tier`, `implied_tier`, `mode`, `delegated_call`, `findings_ok`), so timing, tokens and
free text such as error messages cannot change it.
...
number-free summary of the three answers, uses subheadings and bullet lists, may carry one small
...
capability from knowledge, capability from the session's tools) and one per delivery condition,
each saying use, do not use, or use with a stated caveat, decided only by which claims hold. It
...
- **Judge cost**: about USD 0.10 to 0.22 per judged run with Fable 5.1.
- **Judge vendor**: the judge is Fable 5.1, a Claude model grading Claude models alongside Codex,
  and it has never been checked against human grading. Its scores are evidence of adherence to the
  skill text, not of correctness, and a vendor bias in either direction cannot be ruled out. The
  findings generator is a Claude model too (its model is named in the `FINDINGS.md` header).
...
- **One skill per run.** Competition between similar skills, and selection among many, are untested.
...
- **Product-level vs session-level ground truth** for `harness-stamp`. The ground truth encodes
  product features (`codex features list`: hooks, multi_agent, image_generation all enabled), which
  need not match what a given session exposes. Whether an image tool is exposed in a Codex
  session is unverified. Hooks are not observable from inside either session; both harnesses
  support them. The session-level value for subagents has a different provenance per pair; see
  Ground truth.
- **Judge input excludes bytecode caches.** The scorer imports `src.app` and runs the agent's
  tests, which creates `__pycache__` directories in the workspace before the judge sees it, so
  the judge's diff excludes them; otherwise scope discipline would be docked for files the agent
  never wrote.
